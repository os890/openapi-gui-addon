...
- `oauth2-support_simple-inline-login` — Bearer-token variant with a **custom HTML login form** embedded in the Swagger UI page (via a white-label `openapi.html`). ~80 lines of inline JavaScript, zero dependencies — POSTs the username/password to Keycloak's `/token` endpoint directly and calls `preauthorizeApiKey` on Swagger UI. Swagger UI's default Authorize button is CSS-hidden; users can only sign in and out via the custom UI. Same token shape (password grant) as the bearer branch, but with full UX control and no paste-the-token dialog ever visible.
